...
func DebugPrintError(err error) {
	log.Println("[JINRITEMAI_DEBUG] [ERROR]", err)
}

func DebugPrintStringResponse(str string) {
	log.Println("[JINRITEMAI_DEBUG] [RESPONSE]", str)
}

func DebugPrintGetRequest(url string) {
	log.Println("[JINRITEMAI_DEBUG] [API] GET", url)
}

func DebugPrintPostMapRequest(url string, data url.Values) {
	const format = "[JINRITEMAI_DEBUG] [API] JSON POST %s\n" +
		"http request body:\n%s\n"
...
func DebugPrintPostMultipartRequest(url string, body []byte) {
	log.Println("[JINRITEMAI_DEBUG] [API] multipart/form-data POST", url)
}
...
func DecodeJSONHttpResponse(r io.Reader, v interface{}) error {
	...
	log.Printf("[JINRITEMAI_DEBUG] [API] http response body:\n%s\n", body2)
